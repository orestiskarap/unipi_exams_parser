...
def create_exam_calendar(exam_data):
    ...
    for semester in range(1, 9):
        answer=input(f"Do you want to choose lessons from semester {semester}? (y/n): ")
    ...
    for event_data in exam_data:
        ...
        if event_data['Semester'] in str(semester_selection):
            answer=input(f"\nDo you want to add {event_data['Lesson Name']} from semester {event_data['Semester']} to calendar?(y/n): ")
            while(answer!="y" and answer!='n'):
                answer=input("Please answer with y or n: ")
