...
def create_exam_calendar(exam_data):
    ...
    for semester in range(1, 9):
        ...
        answer=input(f"Do you want lessons from {semester} semester? (y/n)")
        while(answer!="y" and answer!='n'):
            answer=input("Please answer with y or n: ")
        if answer=="y":
            semester_selection.append(semester)
        
    print(semester_selection)
    #create calendar
    c = Calendar()
    ...
    file_name = f"exams.ics"
    # for event_data in semester_events:
    for event_data in exam_data:
    # for event_data in exam_data:
        event = Event()
        ...
        event_uid = hashlib.md5(f"{event_data['Lesson Name']}".encode()).hexdigest()
        event.uid= f"{event_uid}@unipi.com"
        
        # print('event_data["Lesson Name"]: ',event_data['Lesson Name'])
        # print('event_data["Semester"]: ',event_data['Semester'])
        
        if event_data['Semester'] in str(semester_selection):
            answer=input(f"\nDo you want to add {event_data['Lesson Name']} to calendar?(y/n): ")
            while(answer!="y" and answer!='n'):
                answer=input("Please answer with y or n: ")
